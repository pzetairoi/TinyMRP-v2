...
| Ubuntu 24.04 CI, LAN | `lan-mode-plain-http` job: installs against the runner's non-loopback address, asserts `TINYMRP_URL` reaches the container, no `Secure` cookie and no `upgrade-insecure-requests` on a plain-HTTP origin, then drives a real form login through to an authenticated page and checks the demo dataset landed | shares the localhost job's data path | shares the localhost job's lifecycle logic | shares the localhost job's volume handling | Automated on every relevant change. This is the mode that was broken: loopback is a potentially trustworthy origin, so the localhost job cannot detect it |
| Ubuntu/Debian, LAN, second physical device | not yet run on a clean release host | not yet run | not yet run | not yet run | Release checklist |
