param(
  ...
  # The address a browser uses to reach this stack. Its scheme decides whether
  # session cookies are marked Secure and whether the CSP upgrades page assets
  # to https - so an https:// value on a plain-HTTP stack produces a login that
  # loops for ever. Point it at a LAN address to reach the stack from another
  # machine, e.g. -PublicUrl http://192.168.1.50:5000
  [Parameter(Mandatory = $false)]
  [string]$PublicUrl = ""
)
...
try {
  ...
  # Secrets are required and are never generated by the application. Reuse the
  # stored pair when one exists: replacing SECRET_KEY invalidates every session
  # and every signed file link the previous run handed out.
  $secretKey = Get-EnvFileValue $envFile "SECRET_KEY"
  if (-not $secretKey) { $secretKey = New-SecureRandomHex 32 }
  $passwordSalt = Get-EnvFileValue $envFile "SECURITY_PASSWORD_SALT"
  if (-not $passwordSalt) { $passwordSalt = New-SecureRandomHex 32 }

  if (-not $PublicUrl) {
    $storedUrl = Get-EnvFileValue $envFile "TINYMRP_URL"
    if ($storedUrl) { $PublicUrl = $storedUrl } else { $PublicUrl = "http://localhost:$HttpPort" }
  }
  if ($PublicUrl -notmatch '^https?://') {
    throw "PublicUrl must start with http:// or https:// (got: $PublicUrl)."
  }

  @(
    "TINYMRP_DELIVERABLES_DIR=$dockerDeliverables"
    "HTTP_PORT=$HttpPort"
    "TINYMRP_URL=$PublicUrl"
  ...
  Write-Host "Starting containers; TinyMRP will be reachable at $PublicUrl ..."
