...
| SEC-CSP-01 | P1 | Code-complete 2026-08-06; awaiting a browser check | 1A residual/2 | Inline scripts and handlers still require CSP `unsafe-inline`. All 34 inline `on*=` handlers are converted to delegated listeners, all 18 inline `<script>` blocks carry a nonce, and `style-src` is decoupled so `script-src` hardens on its own (self + nonce). `style-src` keeps `unsafe-inline` deliberately: the 86 `style=` attributes are static CSS that cannot execute, and a nonce does not cover style attributes. Remaining: flip `TINYMRP_CSP_ALLOW_INLINE=false` and exercise the admin UI in a browser - ideally with `TINYMRP_CSP_REPORT_ONLY_STRICT=true` first. |
...
| SUPPLY-LOCK-01 | P1 | **Closed 2026-08-04** | 3C/8 | Python install artifacts lack `--require-hashes` provenance lock. `requirements.lock` pins all 60 packages (44 direct + 16 transitive) by sha256; the release image installs with `--require-hashes`, and CI fails if the lock drifts from `requirements.txt`. Verified: a tampered hash is rejected (`THESE PACKAGES DO NOT MATCH THE HASHES`), and the production image builds and imports cleanly. |
| IMPORT-DOS-01 | P0 | **Closed 2026-08-04** (Phase 4A, `5c9473f`) | 4A | No cumulative uncompressed/archive compression-ratio limit. Compressed/per-entry/aggregate/ratio limits, an allowed-compression-method policy, filename normalisation and a deliverable file-type policy are enforced in `parse_import_package`, covered by 12 adversarial tests that build hostile archives in memory. |
| IMPORT-ATOMIC-01 | P0 | Largely mitigated 2026-08-06; still Open | 4B | Cross-store imports can leave partial database/filesystem state. A durable journal, unique operation IDs, compensating rollback of created parts, an idempotency key that recognises repeated imports, and operator endpoints (`/api/import/operations`) are all in place. Still open: no MongoDB transactions, no true resume (a retry redoes the whole pack), no admin UI over the journal, and in-place modifications to pre-existing parts still cannot be auto-restored - they are reported for manual reconciliation. |
| OPS-DBAUTH-01 | P0 | Largely mitigated 2026-08-07; still Open | 5 | Mongo authentication is optional/default-off. New guided instances start authenticated AND the application now connects as a least-privilege user with `readWrite` on its own database only - verified against a real Mongo that reading `admin` and creating users are both refused. Unauthenticated deployments log a startup warning and surface on `/api/ready`. Still open: existing instances are not migrated, the non-guided `docker-compose.yml` still defaults to empty credentials, and no live VPS install has been executed. |
| OPS-HEALTH-01 | P1 | Largely mitigated 2026-08-06; still Open | 5 | Health endpoint does not prove database/storage readiness. All three endpoints now exist: `/api/health` keeps its protected liveness contract, `/api/ready` verifies a real Mongo ping plus storage writability and free disk, and `/api/diagnostics` gives operators detail behind `system.maintenance` with secrets redacted. Still open: nothing consumes `/api/ready` yet - Compose/Caddy healthchecks and deploy scripts still poll `/api/health`, and wiring that needs the protected VPS/Caddy path re-validated end to end. |
...
| QA-FE-01 | P1 | Partially addressed 2026-08-06; still Open | 6 | No frontend unit, browser or automated accessibility tests. Vitest + React Testing Library are wired with 302 tests; all of `src/lib/` is covered at 92% behind a BLOCKING coverage threshold, and a `tsc --noEmit` guard fails CI on unimported identifiers (added after an extraction shipped a broken page). Still open: no Playwright browser workflows, no axe accessibility checks, and 5 of 8 pages have no page-level test. |
